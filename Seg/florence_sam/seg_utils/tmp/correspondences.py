...
def find_correspondences(image_path1: str, image_path2: str, num_pairs: int = 10, load_size: int = 224, layer: int = 9,
    ...
    print("we have create the model, that's good")
    ...
    print("guess we load all the images now")
    ...
    # print("saliency_map1's shape", saliency_map1.shape)
    # print("saliency_map2's shape", saliency_map2.shape)
    # print("we have extract the saliency map!")
    ...
    print("finish similarities compute")
...
def draw_correspondences(points1: List[Tuple[float, float]], points2: List[Tuple[float, float]],
    ...
    ax2.axis('off')
    print("image1 and image2 sizes", image1.size, image2.size)
    ax1.imshow(image1)
    ax2.imshow(image2)
    if num_points > 15:
